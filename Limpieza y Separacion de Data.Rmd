---
title: "De Hojas de Cálculo a Bases de Datos por Separado"
...
Adicionalmente, uso de forma opcional un código poco eficiente -- en el sentido de contener repeticioens -- pero útil para corregir el nombre de las columnas y limpiar la base de caractéres extraños con `stringr` y `stringi`.
...
El código de abajo usa la función `map` y tiene dos parámetros principales: uno referida a los objetos a los que se aplicará una misma función ("input") y otro referida a la función que será aplicada ("function"). La función  es `read_excel` y se aplica a la lista de nombres de hojas de cálculo asignadas por `excel_sheets`. El resultado es una lista de bases de datos, cuyos  nombres correspondientes se asignan con `set_names`. Cabe destacar que `file.choose()` permite asignar el archivo de Excel "modelo_raw_data.xlsx" abriendo *finder*, pero puede hacerse de otras maneras. 
